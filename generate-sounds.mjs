...
function getApiKey() {
  ...
  if (existsSync(envFile)) {
    const m = readFileSync(envFile, "utf8").match(/^\s*FISH_AUDIO_API_KEY\s*=\s*(.*)$/m);
    if (m) return m[1].trim().replace(/^["']|["']$/g, "");
  }
  throw new Error("FISH_AUDIO_API_KEY not set (use an env var or a .env file next to this script).");
}

async function tts(text, referenceId, apiKey) {
...
for (const [voice, def] of Object.entries(VOICES)) {
  ...
  if (/REPLACE_WITH/.test(def.reference_id)) {
    console.error(`  ! "${voice}" still has placeholder reference_id — set a real Fish Audio model id`);
    continue;
  }
...
console.log(`\n[gen] done. ${ok} ok, ${skip} skipped, ${fail} failed. -> ${OUT_ROOT}`);
